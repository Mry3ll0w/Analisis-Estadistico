...
```{r, echo=TRUE}
# 1. Obtener probabilidades en el conjunto test
probabilidades <- predict(modelo_logistico_v2, newdata = test, type = "response")

# 2. Aplicar el punto de corte de 0.5
predicciones_clase <- ifelse(probabilidades > 0.5, "Alto", "No Alto")

# 3. Convertir a factor con los mismos niveles que la variable original
predicciones_clase <- factor(predicciones_clase, levels = c("No Alto", "Alto"))

# 4. Crear la Matriz de Confusión
matriz <- table(Real = test$SalarioAlto, Predicho = predicciones_clase)

# 5. Mejoras visuales de la matriz
# Convertir tu matriz en un data frame para ggplot
matriz_df <- as.data.frame(matriz)

ggplot(data = matriz_df, aes(x = Predicho, y = Real, fill = Freq)) +
  geom_tile(color = "white") +
  geom_text(aes(label = Freq), vjust = 1, size = 4, color = "white") +
  scale_fill_gradient(high = "#EB9573", low = "#B5E3AC") +
  labs(title = "Matriz de Confusión: Predicción de Salario Alto",
       subtitle = "Punto de corte = 0.5",
       x = "Predicción del Modelo",
       y = "Valor Real (Test Set)") +
  theme_minimal()

```
  
  
Basandonos en lo mostrado por la matriz de confusión podemos afirmar lo siguiente:  

1. __Aciertos (Diagonal Principal):__ El modelo identificó correctamente a 793 personas con salario "No Alto" (Verdaderos Negativos) y a 479 con salario "Alto" (Verdaderos Positivos).


2. __Errores (Fuera de la diagonal):__

- 165 Falsos Positivos: Personas que el modelo predijo con salario alto, pero no lo tienen.

- 240 Falsos Negativos: El modelo "perdió" a estas personas que realmente ganan mucho. Esto coincide con la tendencia del modelo a infravalorar los ingresos más altos vista en los gráficos de residuos.


Procedemos ahora a calcular las Métricas de Precisión:  

1. __Exactitud__: 
- $Total = 793 + 165 + 240 + 479 = 1677$ 
- $Aciertos = (793 + 479)/1677 = 0.758$
- El modelo acierta el $\approx 76%\%$ de las clasificaciones.


2. __Sensibilidad__:

- $Sensibilidad = 479 / (479 + 165) = 0.667$
- El modelo es capaz de detectar al 66.6% de los que ganan mucho.

3. __Especifidad__:
- $Especifidad = 793 / (793+165) = 0.828$
El modelo es muy bueno (82.8%) identificando a quienes no tienen salarios altos.


### 2.5. Predicción

```{r, echo=TRUE}
# 1. Probabilidades calculadas para el test
prob_test <- predict(modelo_logistico_v2, newdata = test, type = "response")

# 2. Añadir la probabilidad al conjunto test para poder visualizarla
test_con_prob <- cbind(test, Probabilidad = prob_test)

# 3. Ordenar de mayor a menor probabilidad y seleccionar los 5 primeros
top_5_perfiles <- test_con_prob[order(-test_con_prob$Probabilidad), ]
top_5_perfiles <- head(top_5_perfiles, 5)

# 4. Seleccionar las columnas clave para la tabla
columnas_interes <- c("SEXO", "ESTU", "ANOANTI", "JAP", "CNO1_grp", "Probabilidad")
tabla_final <- top_5_perfiles[, columnas_interes]
...
# Cálculo diferencia de devianzas
dif_devianza <- modelo_logistico_v2$null.deviance - modelo_logistico_v2$deviance

# Diferencia grados de libertad
dif_df <- modelo_logistico_v2$df.null - modelo_logistico_v2$df.residual

# p-valor
p_valor_global <- pchisq(dif_devianza, df = dif_df, lower.tail = FALSE)

print(p_valor_global)

```
Con esto echazamos la hipótesis nula de que el modelo no aporta nada. El conjunto de variables explicativas tiene un efecto real y potente sobre la probabilidad de tener un salario alto.
Por tanto se concluye que el conjunto de variables (SEXO, ESTU, ANOANTI, etc.) explica la probabilidad de tener un salario alto de forma mucho más eficaz que el simple azar o el promedio.  

```{r,echo=TRUE}

curva_roc <- roc(test$SalarioAlto, probabilidades)

# Dibujar la curva
plot(curva_roc)
# Calcular el AUC
auc_resultado <- auc(curva_roc)
print(auc_resultado)
...
resumen_resultados <- data.frame(
  Pregunta = c("Determinantes", "Brecha Género", "Ocupación", "Fiabilidad", "Detección"),
  Estadistico = c("OR ESTU6: 22.5", "OR Mujer: 0.317", "OR CNO3: 0.254", "Exactitud: 75.8%", "Sensibilidad: 66.6%"),
  Conclusion = c("Educación es el mayor protector", "Penalización femenina severa", "Baja cualificación es riesgo", "Modelo robusto y significativo", "Mejor prediciendo salarios bajos")
)

kable(resumen_resultados, caption = "Resumen de Resultados del Modelo Logístico")

```


Aunque el modelo presenta una eficacia global excelente y una alta capacidad discriminatoria ($AUC$ robusto), su mayor precisión para identificar salarios bajos sugiere la existencia de variables no observadas que influyen en la movilidad hacia los niveles de renta más elevados.

### 3.2. Conclusiones y Reflexión Ética


El estudio revela que la desigualdad salarial es un fenómeno complejo que no puede explicarse solo por la formación o la experiencia. Al comparar el modelo lineal simple con el múltiple, observamos que la brecha "bruta" inicial se reduce al incluir variables como el nivel educativo o la antigüedad, dando lugar a la brecha "ajustada". Sin embargo, incluso tras este ajuste, ser mujer sigue siendo un factor de riesgo que reduce las probabilidades de éxito salarial en un 68.3%.

Existen limitaciones críticas: los modelos no capturan variables como los cuidados no remunerados, la discriminación directa o las habilidades específicas. Además, usar el "sexo registrado" es una aproximación limitada del género. Éticamente, esto implica que la brecha detectada es solo la "punta del iceberg"; el modelo confirma una penalización estadística injustificada, pero los sesgos de los datos podrían estar infravalorando la magnitud real de la segregación laboral.
